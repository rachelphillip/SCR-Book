...
\ei
To get a feel for SPPPs, let's look at some pictures in the context of the Kruger leopard survey. Figure~\ref{fig:spmod-KrugerDs} shows the intensity of the SPPP over all $\bm{s}$ in the survey region within an area $A$, estimated as a function of habitat suitability. We refer to such maps as "desity surfaces", denoted $D(\bm{s})$. 
