...
\todo[inline]{density surfaces not previously defined. I think Royle et al. have refered to posterior distribution of activity centers (with no covariates) as a "density surface". I think we should make it clear that we only see variable D within A if we include spatial covariates. 
