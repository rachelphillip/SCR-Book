...
\todo[inline]{I edited the above because density surfaces were not previously defined. I think Royle et al. have refered to the posterior distribution of activity centers (with or without spatial covariates) as a "density surface". I think we should make it clear here that we only get maps of spatial variation in D within A if we include spatial covariates.}
