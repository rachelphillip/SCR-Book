...
\ei
To get a feel for SPPPs, let's look at some pictures in the context of the Kruger leopard survey. Figure~\ref{fig:spmod-KrugerDs} shows the intensity of the SPPP (i.e. leopard density) over all $\bm{s}$ in the survey region within an area $A$, estimated as a function of habitat suitability. We refer to such maps as "desity surfaces", denoted $D(\bm{s})$. 
\todo[inline]{I edited the above because density surfaces were not previously defined. I think Royle et al. have refered to teh posterior distribution of activity centers (with or without spatial covariates) as a "density surface". I think we should make it clear here that we only get maps of spatial variation in D within A if we include spatial covariates.}
\todo[inline]{Should it be denoted D-hat(s)?}
\todo[inline]{Here as in Ch 2, we include spatial variation in our figures before introducing it in the text. I think the figures should illustrate the concepts described, not concepts to be described subsequently. Remove Kruger figs. to subsequent section?}

\begin{figure}[ht]
\caption{\small The density surface $D(\bm{s})$ for the Kruger leopard data, as a heatmap (left) and a perspective plot (right). Camera locations are shown as white crosses in the left plot. The survey area (A) is outlined in black.}
\centering
...
\todo[inline]{Could we either integrate the complete example into sections 3.2 and 3.3, or remove the Kruger figures from section 3.2 and present them here instead? We could plot one flat density surface, and make it clear earlier on that we can't get an "interesting" density surface without covariate info (and that this covariate info is attached to the mask, if not mentioned in the previous chapter)?
...
\todo[inline]{I think it would be helpful present this earlier on, perhaps as part of or immediately following section 3.2.?}
