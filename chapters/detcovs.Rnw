\chapter{Incorporating covariates in encounter rate and detection probability}
\label{ch:detcovs}

\abstract{This chapter concerns the dependence of encounter rates and detection probabilities on covariates. It shows how they can be made to depend on survey duration and survey effort, on detector, occasion and session, on individual-level covariates (behavioural response and sex, for example), and importantly, on spatial covariates. It also deals with the interpretation of models fitted with these kinds of dependence.}
...
The hazard rate form for mean encounter rate (Eqn~\ref{eq:ERdetfun-negexp-lambda}) cannot be written in this form, while the hazard rate from for detection probability can only be written in this form if $g_0=1$ (with an inverse cloglog link and linear predictor $\beta_0 + \beta_d\log(d)$, in which $\beta_0=b\log(\sigma)$ and $\beta_d= -b$). The Gaussian detection function model cannot be written in this form. 
...
Moving has a cost (energetically, at least), so that distance travelled (by individuals, or in the case of acoustic surveys, by sound) is a measure of cost. The farther the movement, the greater the cost. When cost is measured only by distance, the least cost path between any two points is just the shortest path between them (i.e. a straight line between them) and the least-cost distance is the straight-line distance between them (or great circle distance if the distance is big enough that curvature of the earth matters). So all SCR models involve using a least-cost path and a least-cost distance, albeit a particilarly simple one.

Note that the scale parameter, $\sigma$, scales the distance $d$ by dividing it. (This is what a scale parameter does, by definition.) The larger $\sigma$, the farther the individual moves. For example, an individual with $\sigma=2$ has a home range radius twice that of an individual with $\sigma=1$. So we could say that in some sense the cost to an individual with $\sigma=2$ of moving any distance $d$ is half the cost of moving this same distance, to an individual with $\sigma=1$. With this interpretation, $d/\sigma$ is the cost of travelling a distance $d$ for any individual, and $\sigma^{-1}$ is the cost per unit distance, i.e. the rate at which cost accumulates per distance unit travelled.

Things get more difficult and more interesting when this cost per unit distance changes depending on where the individual is at the time -- if it depends on the habitat at each point in space, for example.
...
Efficient methods for finding least cost paths are not trivial, but fortunately the \texttt{R} libraries \texttt{igraph} and \texttt{gdistance} (which uses \texttt{igraph}) implement efficient methods to find least cost paths, using Dijkstra's algorithm, \todo{insert ref} so the user need not worry about the method.

The cost rate function, however, is up to the user to define. With the above definition of cost, the cost of travelling between $\bm{s}_m$ and $\bm{s}_{m^*}$ is  
\be
...
\cite{Royle+al:13a} and \cite{Sutherland+al:14}\todo{And others?} used the following cost rate function:
\be
c(\bm{s}_m,\bm{s}_{m^*})&=&\exp\left\{\alpha\frac{z(\bm{s}_m)+z(\bm{s}_{m^*})}{2}\right\},
\label{eq:detcovs_royle_cost_fn}
\ee
\noindent
where $z(\bm{s}_m)$ and $z(\bm{s}_{m^*})$ are the values of some habitat variable that is expected to affect the cost of travelling between neighbouring cells $\bm{s}_m$ and $\bm{s}_{m^*}$, and $\alpha$ is a parameter that quantifies the strength of the effect. They call $d^{(c)}(\bm{s}_m,\bm{s}_{m^*})$ the ``ecological distance''. Note that when $\alpha=0$ the ecological distance is equal to the Euclidean distance, and that $\alpha$ determines the strength of the effect of the spatially referenced covariate $z$ on the cost of movement. Usually $\alpha$ would be unknown and would be estimated from the SCR survey. An estimate of $\alpha$ has a useful ecological interpretation as a measure of the strength of the effect of $z$ on the cost of movement between neighbouring cells. 

Figure~\ref{fig:detcovs-igraphplot} illustrates the implementation of a cost rate function on a simple 4$\times$4 cell discretisation of space. In the left plot, dark shaded cells have $z(\bm{s})=1$, light shaded cells have $z(\bm{s})=100$ and white cells are excluded (individuals can't move into them). The set of cells is represented as a network in the right plot of Figure~\ref{fig:detcovs-igraphplot}, with vertex number corresponding to cell number in the left plot, neighbouring cells are joined by lines, and the ecological distance between neighbours, using the cost rate function shown in Eqn~\eqref{eq:detcovs_royle_cost_fn}, shown on each line. The least-cost path between cells 16 and 2 is shown in Figure~\ref{fig:detcovs-lcpath}. 
\begin{figure}[ht]
...
The cost rate function function defined by Eqn~\eqref{eq:detcovs_royle_cost_fn} will not be appropriate for all situations. Notice, for example, that this function is symmetric: the cost of moving from $\bm{s}_m$ to $\bm{s}_{m^*}$ is identical to the cost of moving from $\bm{s}_{m^*}$ and $\bm{s}_m$. If $z(\bm{s})$ was elevation, for example, a cost rate function in which the cost of going uphill from $\bm{s}_m$ to $\bm{s}_{m^*}$ was greater than the cost of going downhill from $\bm{s}_{m^*}$ and $\bm{s}_m$ seems more appropriate. One might prefer a function similar to that below in this case:
\be
c(\bm{s}_m,\bm{s}_{m^*})&=&\exp\left\{\alpha[z(\bm{s}_m)-z(\bm{s}_{m^*})]\right\},
\label{eq:detcovs_elev_cost_fn}
\ee

But this is detail -- the basic idea of using ``ecological distance'' instead of straight-line distance to measeure the distance between points in space from the target species' perspective is a powerful one. It facilitates a number of useful ecological interpretations, including inferences about landscape connectivity, better reflects individuals' behaviour in heterogeneous environments, and \cite{Sutherland+al:14} showed that using Euclidian distance in highly structured environments when individuals move according to an ecological, non-Euclidian, distance, results in negatively biased density and abundance estimates. 

The is no one-size-fits-all cost rate function, and it is up to the user to define sensible cost rate functions for their applications, but the option to do so adds very substantially to the power of SCR analyses. You can select between cost rate functions using likelihood-based model selection criteria such as AIC or AIC$_c$, in the same way that you can select between encounter rate functions, detection functions, or density surface models. 
