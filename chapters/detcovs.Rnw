...
Heterogeneity in detection probabilities, particularly heterogeneity among individuals attributable to unobserved covariates, has always been regarded as problematic in non-spatial CR studies. With SCR, encounter rate or detection probability is alwasys a function of the distance between each individual's activity center and detectors, so the models explicitly accounts for a key unobserved source of heterogeneity among indivduals - spatially variable exposure to detectors. 
Other sources of heterogeneity can be modeled using covariates and mixture distributions.
