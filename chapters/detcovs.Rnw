...
Due to the inherent variability of natural systems, SCR model parameters describing the detection process are likely to vary in space, time, and among individuals. Modeling this heterogeneity adds complexity and usually requires larger samples, but ignoring it can cause biased estimation and flawed inference. Unexplained heterogeneity in detection probabilities, especially among individuals, has always been regarded as problematic in CR studies. With SCR, encounter rate or detection probability is alwasys a function of the distance between each individual's activity center and detectors, so SCR models explicitly account for one important source of heterogeneity among indivduals: spatially variable exposure to detectors. 
This chapter describes how other sources of heterogeneity can be modelled using covariates. SCR models accommodate covariates specific to detector locations, sampling occasions, sessions, and individuals. Indivdual heterogeneity can be modelled several ways, including using unobserved or partially-observed covariates. An important extension further allows modelling of the distance between activity centers and detectors as an effective distance that reflects variable costs of movement through intervening habitats (wording?).

An evaluation of detection heterogeneity begins with a set of candiate models with different sources and/or patterns of variation in detection parameters. MS criteria can then be used to evaluate support for specific covaraites and models. Estimation of population density is the goal of most SCR studies. However, models of the detection process represent biological hypotheses, including regarding differences in home range size, which may also be of interest.  

We begin this chapter by explaining how SCR parameters may be expressed as functions of covariates. We then present and demonstrate methods for modelling increasingly complex covariate effects.
...
\todo[inline]{Point out that some covariates might not be appropriate to both parameters, and different covariate models represent hypotheses? E.g. camera trap model as a covariate for the intercept but not the scale; factors that affect extent of movements (e.g. season) as covariate for sigma.}  
...
\section{Detector- occasion- and session-specific effects}
\label{sec:ERdetfun-dos-effects}

\todo[inline]{Term "session" already introduced, so use in place of "strata"?}
...
Detector-specific covariates are actually specific to the locations of detectors, and may include, for example, the habitat type or characteristics at the location, and any covariates specific to the detector itself (camera trap model).  They can remain the same at each location for the survey duration, or vary with occasion and/or stratum (see Table~\ref{tab:ERdetfun-detdata}). In addition, encounter rate and detection function models can be made to depend on occasion and/or session in the same way for all detectors, by using occasion number or session name as covariates.

Occasion-specific covariates reference differences in time, increasing sigma with season, effects of variable weather conditions on DNA degredation.

Sessions often represent different surveys separated in space and or time, but can also be used to model differences between subsets of the population (sexes or social classes). Refer to sections 2.6 and 4.4 here?
...
\todo[inline]{Eric: in response to the comment above, I've added some of what I think ecologists will want to know about modeling effort. It's probably too much especially re: censoring data.}

As we have seen, SCR models are parameterized in terms of the duration of samplnig at specific locations. Sampling effort at each location must therefore be accurately quantified for the models to yield accurate results. If the input data overestimate sampling effort, density estimates will be negatively biased, and vice versa. Any time a detector is moved or fails to function, the sampling duration at the associated locations is less than the survey duration and must be specified in the model. SCR models allow us to specify, for each location where a detector operated: (1) the total duration of sampling, (2) whether or not it operated for the entirety of each occasion, or (3) the duration of sampling on each occasion, depending on the type of detector (see Ch 5) and the information available.   
...
Sampling effort at each location is conveniently modeled as a detector-level covariate. 
...
\todo[inline]
{Eric: I agree that we should keep density constant here. I don't agree with using AICc to show that modelling effort provides a better fit. That's fine for other covariates, which are usually hypothesized effects, but shouldn't effort always be accurately quantified in the model? Regarless of what AICc says? Modelling effort doesn't add parameters does it? Also, the Tai chimp study is now in press. We could ask for permission to use those data.}
...
The difficulty with covariates like sex or age, that are attached to individuals, is that we don't know them for individuals we did not detect. There is, however, one individual-level covariate that we know for all individuals, and it is useful to classify the individual-level covariates according to which individuals have known and which have unknown covariates, as follows:
\ben
...
\todo[inline]{Not sure if this nice lead-in is still effective if we introduce trap responses above}
...
\todo[inline]{Eric: bk seems realistic when traps are baited with a food reward, as in many carnivore studies. Or, even with no food reward, if an animal is wary of evidence of human disturbance, but unconcerned by a camera trap, it might react differently to camera trap placements it has and has not investigated previously).}
...
\todo[inline]{We had a section called trap response, so I think we should have one called Individual heterogeneity, with subsubsections for different ways of modelling it}

Differences in encounter rate or detection probability among individuals other than trap responses can be modelled several different ways, depending on whether covariates are categorical or continuous, and whether they are completely observed, unobserved, or parially observed.

\subsubsection{Observed covariates}

Can model categorical individual covariates using sessions or groups.
Categorical or continuous individual covariates via conditional likelihood (can't model spatial variation in D within sessions).
Quick example showing identical estimates (or redundant? Ben?)

\subsubsection{Unobserved covariates}
...
\subsubsection{Partially observed covariates}
...
\subsubsection{Time-varying unobserved or partially-observed covariates}
...
\subsection{Spatial effects at detectors}
...
\subsection{Spatial effects away from detectors}
