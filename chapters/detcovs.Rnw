...
\abstract{This chapter concerns the effect of covariates on encounter rates and detection probabilities. It shows how to model their dependence on survey duration and survey effort, and on covariates, which can be specific to sampling occasions, sessions, individuals, and locations, including both sampling locations and intervening habitat. It also deals with the interpretation of models fitted with these kinds of dependence.}
...
Due to the inherent variability of natural systems, SCR model parameters describing the detection process are likely to vary in space, time, among individuals, and posibly in response to detection. Modeling this heterogeneity adds complexity and requires larger samples than would otherwise be needed, but ignoring it can cause biased estimation and flawed inference. Unexplained heterogeneity in detection probabilities, especially among individuals, has always been regarded as problematic in CR studies. With SCR, encounter rate or detection probability is alwasys a function of the distance between each individual's activity center and detectors, so the models explicitly account for one important source of heterogeneity among indivduals: spatially variable exposure to detectors. 
This chapter describes how other sources of heterogeneity can be modelled using covariates. SCR models accommodate covariates specific to detector locations, sampling occasions, sessions, and individuals. Indivdual heterogeneity can be modelled several ways, including using unobserved or partially-observed covariates. An important extension further allows modelling of the distance between activity centers and detectors as an effective distance that reflects variable costs of movement through different intervening habitats (wording?).
...
We begin by explaining how SCR parameters can be, and are, are expressed as functions of covariates (intercept/scale and link function). We then present and demonstrate methods for modelling increasingly complex covariate effects.
...
Incorporating detector-specific, occasion-specific and session-specific covariates in expected encounter rate or detection function models is done by simply adding terms to the appropriate linear predictor (after importing data like that shown in Table~\ref{tab:ERdetfun-detdata}). If one uses the \{intercept; scale\} form, then the intercept and the scale parameters can separately be made to depend on covariates, using the intercept-specific and scale-specific \{link function; linear predictor\} forms shown in Table~\ref{tab:ERdetfun-invlinks}. An alternative is to add the covariates to a \{link function; linear predictor\} model for the encounter rate or detection function itself. This is much less common in the SCR literature.
"Detector-specific" covariates are actually specific to the sampling locations. can remain the same at each detector for the survey duration, or vary with occasion and/or stratum (see Table~\ref{tab:ERdetfun-detdata}). In addition, encounter rate and detection function models can be made to depend on occasion and/or stratum in the same way for all detectors, by using occasion number or stratum name as covariates.

\todo[inline]{Is it acceptable to use sessions to subdivide the data among individuals e.g. sexes, or age/sex classes, if age/sex class is always observed for detected animals? People do. The same effect can be achieved using "groups" in 'secr'. This comes back to the question of how important it is that sessions are completely independent.
If so, potential to model sex variation in one data set using: sessions, groups, conditional likelihood, unobserved covariates, partially observed covariates. Demonstrate that the first 3 methods yield the same estimates, different estimates with larger variance from the last 2.}
...
\subsection{Indivdual heterogeneity}
\todo[inline]{If we have a section called trap response, this should fall under one called Individual heterogeneity}

Differences in encounter rate or detection probability among individuals other than trap responses 

\subsubsection{Conditional likelihood} 
\todo[inline]{Makes sense to cover this before introducing mixtures? Include continuous covariate e.g. weight where baited live-traps?}
...
\todo[inline]{Need to mention pitfalls, especially with sparse data. Effects not detected --> biased but apparently precise. Detected--> wide confidence limits, possibly false minima and unrealistic estimates --> no reliable estimate.}
With larger samples there is more information available both to detect, and to model, heterogeneity in detection parameters. If density is estimated from simple models becuse the data are insufficient to support greater complexity, there is potential for the estimates to be biased but apparently precise. If detectable, need sufficient data to inform estimation of the effect, variances and CIs can become too wide to be useful. Further increases in sample size --> more forms of het detected and must be modelled... but tapering effects, different models that include most important effects yield similar estimates. With very rich data different models yield the same accurate result (Tai chimp study). With sparse data simple models yield conservative estimates with less than nominal CI coverage.
