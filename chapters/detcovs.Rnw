...
Individual heterogeneity requires more complex modelling approaches because covariate values are unknown for the animals that are not detected. 
...
We begin this chapter by explaining how SCR parameters can be, and are, are expressed as functions of covariates <intercep/scale & link function). We then present and demonstrate methods for modelling increasingly complex covariate effects.
