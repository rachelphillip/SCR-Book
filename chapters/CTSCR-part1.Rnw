...
\section{Time-to-detection data}
...
\todo[inline]{Chpt 1 links det fn with encounter rate model, implicitly assuming constant hazard. Need to show that essentially the same thing with hazard but that needs integrated hazard notation to be more general (ratehr than just $\exp{-\lambda(d) T}$).}

\todo[inline]{Can put in a little more maths to show how shape of det fn is no lomnger normal for different durations?}
