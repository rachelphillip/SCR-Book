...
\todo[inline]{Should I modify the notation to link this expression to $\Omega$}
...
As explained in Section \ref{sec:det-types-independence}, capture in either a multi or single-catch trap precludes capture in any other trap during that occasion and leads to a dependence between encounters across the traps (within an individual). The competition between traps for individuals leads to a competing risks formulation for multi-catch traps but single-catch traps have the additional complexity that, once they are full, they are effectively unable to catch any other individuals and hence the assumption of independence between individuals is also violated (see Table \ref{tab:det-types-table}). 

A suitable capture model for single-catch traps therefore needs to account for a second kind of competing risk, that of competition amongst individuals for traps \citep{Efford09b}. However compared to multi-catch traps, it is considerably more complicated to construct a suitable likelihood \citep{Borchers12, Borchers16} for single-catch traps. To date no DT likelihood function for single-catch traps exists \citep{Efford09b, Royle13, Borchers16, Borchers16b} and consequently the DT multi-catch trap estimator is typically used for the analysis of single-catch trap data. 

As discussed in Section \ref{sec:det-types-single-multi} \todo[inline]{at the moment I am referring to Section \ref{sec:det-types-single-multi} but not sure if some of that detail should rather come here?}, the multi-catch trap estimator appears to be extremely robust regarding the estimation of average density, despite the fact that it assumes independence between individuals that does not hold. However simulations have shown that there are situations when density varies in space and the multi-catch estimator is not able to accurately identify areas of high and low density \citep{Distiller15}. Furthermore the incorrect estimator is negatively biased for detection function parameters ($g_0$ or $\lambda_0$) and there are instances where the accurate estimation of these parameters are of interest in their own right, for example when the detection function is used in movement or space-use models.

For situations in which actual capture times are recorded, the CT setup enables the construction of an appropriate single-catch trap likelihood with only small modifications to the general CT likelihood. In essence, a model that uses exact capture times allows one to disable traps when they are occupied and hence to incorporate the competition amongst individuals for traps.

To construct a likelihood with these features, we define an indicator variable $a_j(t)$ that is 1 if trap $j$ is unoccupied at time $t$ and zero otherwise ($j=1,\ldots,J$), and we define another indicator variable $v_i(t)$ to be 1 if individual $i$ is not in a trap at time $t$, and zero otherwise ($i=1,\ldots,n$). (These variables are readily calculated from the observed capture and release times of individuals at each trap.) The hazard function for individual $i$ for trap $j$ at time $t$ is then conveniently written as  $v_i(t)a_j(t)h_j(t,\bm{s}_i;\bm{\theta})$ from which it can be seen how the two indicator functions ``switch'' the hazard of detection on and off so that it is zero when capture is not possible.

The survivor function across all traps for individual $i$ to time $t$ is therefore defined to be 

\begin{equation}
S^*_\cdot(t,\bm{s}_i;\bm{\theta})=\exp\left(-\int_0^{t} v_i(u)\sum_{j=1}^J a_j(u)h_j(u,\bm{s}_i;\bm{\theta})\;du\right)
\label{eqn:Ssingle}
\end{equation}

\todo[inline]{At the moment this section is a lot shorter than what I had originally envisaged and I'm not sure if this is sufficient or if it should be expanded. I'm not sure how to provide any technical details without showing the likelihood and it seems that we have avoided that up to now. I could also include further details / plots from the simulations I ran but thought that perhaps a reader would just be interested in the main point, and the reference is there for any interested party. It may also be that to show some of the sims I need to provide a whole lot of other detail about how they were run etc. 

I'm also not sure if I should include an example on single-catch traps. Obviously there is not much real data and so the example would really just demonstrate the code to fit these models to simulated data. One downside to excluding this is that then there would not be a function available to users should they have the right kind of data and want to fit the CT model? }
...
Calc.AICc(mod.objs_=list(Jags.IndHaz.K4, Jags.IndHaz.K6, Jags.IndHaz.K8),
...
<<DensEsts2, eval = TRUE>>=
exp(Jags.DepHaz.K4$estimate[1])*10000 ; 
exp(Jags.DepHaz.K6$estimate[1])*10000 ; 
exp(Jags.DepHaz.K8$estimate[1])*10000 
@
...
<<AICs2, eval = TRUE>>=
Calc.AICc(list(IndHaz.K6.common, IndHaz.K6.sep), 
