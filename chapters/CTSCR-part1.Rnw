...
trap.objs_ = Traps, mask.files_ = Mask, haz.type_= 'IndU', haz.k_ = c(4), 
...
<<AICs1, eval = TRUE>>=
aics <- Calc.AICc(mod.objs_=list(IndHaz.K4, IndHaz.K6, IndHaz.K8), decimals_ = 2, n.inds_ = 17)
print(xtable(aics, digits=2, 
             caption="AICc's from independent hazard models.",
             label="CTAICc1"))
@

The \texttt{plot.hazard} function plots the estimated detection hazard for a specified model. Note the \texttt{add\_} argument that can add lines to an existing plot and the \texttt{col\_} argument to choose colours:

\begin{verbatim}
plot.hazard(det.pars_ = c(IndHaz.K6$estimate[2:3]), 
spline.pars_ = c(IndHaz.K6$estimate[4:7]), haz.k_ = 6, 
haz.type_ = 'IndU', scaled_ = T, add_ = F, col_ = "blue")
plot.hazard(det.pars_ = c(IndHaz.K4$estimate[2:3]), 
spline.pars_ = c(IndHaz.K4$estimate[4:5]), haz.k_ = 4,
haz.type_ = 'IndU', scaled_ = T, add_ = T, col_ = "red")
plot.hazard(det.pars_ = c(IndHaz.K8$estimate[2:3]), 
spline.pars_ = c(IndHaz.K8$estimate[4:9]), haz.k_ = 8, 
haz.type_ = 'IndU', scaled_ = T, add_ = T, col_ = "brown")
...
You can see how the estimated hazard shape changes as K increases. The model that uses a hazard with K = 6 has the lowest AIC and hence a hazard with a crepuscular (active during twilight) shape is deemed most appropriate for modelling detection of jaguars in the Cockscomb Basin Wildlife Sanctuary in Belize. This is in line with \citet{Harmsen11} who reported a crepuscular peak in activity for jaguars between 6 and 7 pm. 

The plots above specified \texttt{scaled\_ = T} and show the scaled hazards for a 24 hour hazard cycle i.e.\ there is no effect of distance and the focus is really on the shape of the hazard function (the $h(t)$ component). The \texttt{plot.hazard} function can also plot the expected encounter rate through time for a given distance (from an activity centre) by changing the \texttt{scaled\_ = T} argument to ``F'' and by supplying a value (or a vector) for distance via the \texttt{dist\_} argument. The \texttt{time.vec\_} argument specifies the time period that the plot should apply to and the `Prob' value shown in the legend that is produced is the probability of being caught at least once over that duration of time. 

\begin{verbatim}
plot.hazard(time.vec_ = c(0,48), det.pars_ = c(IndHaz.K6$estimate[2:3]), 
spline.pars_ = c(IndHaz.K6$estimate[4:7]), haz.k_ = 6, haz.type_ = 'IndU', 
scaled_ = F, add_ = F, dist_ = c(1000, 5000), col_ = c("red", "blue"), 
legend.pos_= "topleft")
\end{verbatim}
...
trap.objs_ = Traps, mask.files_ = Mask, haz.type_= 'Dep', haz.k_ = c(4), 
endpoint_ = 2160, mesh.size_ = 20, hessian=T, trace=FALSE))
\end{verbatim}
...
Plotting the estimated encounter rate function is a little more complicated because the expected encounter rate now depends on both time (i.t.o the hour of the cycle) and distance. If we want to view a two-dimensional plot of the effect of time (or distance), we need to do this for a specified value of distance (or time). 

Using the model with K = 6, in order to generate plots of the expected encounter rate through time the \texttt{dim\_} argument needs to be set as ``time'' and a value for distance supplied via the \texttt{dist\_} argument. To look at the effect of distance the \texttt{dim\_} argument is changed accordingly, a value of time is supplied via \texttt{time\_}, and a vector of distances needs to be supplied to determine what range of distances the plot should use:

\begin{verbatim}
plot.hazard(det.pars_ = c(DepHaz.K6$estimate[2]), 
spline.pars_ = c(DepHaz.K6$estimate[3:7]), haz.k_ = 6, 
haz.type_ = 'Dep', dim_ = 'time', dist_ = 2000, add_ = F, col_ = "blue")
plot.hazard(dist.vec_ = (0:7000), det.pars_ = c(DepHaz.K6$estimate[2]),
spline.pars_ = c(DepHaz.K6$estimate[3:7]), haz.k_ = 6, haz.type_ = 'Dep', 
dim_ = 'distance', time_ = c(2,13,20), dist_ = 2000, add_ = F, 
col_ = c("blue", "red", "green"))
\end{verbatim}
...
The plotting function will produce a 3-dimensional plot if the\texttt{dim\_} argument is set to be ``NULL''. The default is to produce a static 3D plot but a rotatable plot will be produced if the \texttt{rotate\_} argument is set to be ``T''.

\begin{verbatim}
plot.hazard(time.vec_ = 0:24, dist.vec_ = seq(0,7000, by = 10), 
mesh.size_ = 50, det.pars_ = c(DepHaz.K6$estimate[2]), 
spline.pars_ = c(DepHaz.K6$estimate[3:7]), haz.k_ = 6, 
haz.type_ = 'Dep', dim_ = NULL, rotate_ = F)
\end{verbatim}
