...
We deal with each of these sources of difference in expected encounter rate in Chapter~\ref{ch:detcovs}. Here we start by expanding on the idea of a spatial encounter rate.
...
We fit this model using the \texttt{secr} function \texttt{secr.fit}, by passing the function the capture history object \texttt{kruger.capt} (which contains in it details of detector locations), the model specification in the form of \texttt{er.model} and \texttt{detfn} below, and information on the region that could contain the activity centres of detected individuals (\texttt{kruger.mask}).
...
df.hr.fit <- secr.fit(kruger.capt, mask=kruger.mask,
                      model=list(lambda0~1,sigma~1), detectfn="HR")
@
}
