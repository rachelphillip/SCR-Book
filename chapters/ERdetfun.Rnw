...
The necessary data and fitted objects are obtained by loading the library \texttt{scrmlebook} package and getting the data and other \texttt{R} objects for this chapter, as follows:
{\small
<<ER-detfun-leopards-load, include = TRUE, echo = TRUE>>=
library(scrmlebook)
data("ERdetfun-objects")
@
}

This loads the count capture history in \texttt{R} object \texttt{kruger.capt}, the corresponding binary capture bistory in \texttt{kruger.capt.bin} and the mask object in \texttt{kruger.mask} These are all objects that were created using the \texttt{secr} library.
...
\ee
\noindent 
It has one more parameter than a Gaussian model and is hence more flexible. Its shape can be seen in Figure~\ref{fig:ERdetfun.hrfit}. 
...
er.bin.hn.fit=secr.fit(kruger.capt.bin,model=er.hn.model,mask=kruger.mask,
                   detectfn=detfn)
@
} 
...
plot(df.hr.fit,xval=seq(0,20000,length=200),lwd=3,xlab="Distance, d (m)",
     ylab=expression(lambda(d)))
plot(df.bin.hr.fit,xval=seq(0,20000,length=200),add=TRUE,lwd=3,lty=2,
     col="gray")
@
} 
