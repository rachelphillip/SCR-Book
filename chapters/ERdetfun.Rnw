...
\todo[inline]{reference chapter 4 re: variable effort?}
...
\todo[inline]{Perhaps point out that the smaller, binary data set is less informative regarding which detection function model fits best, since AICc values were similar here?}
