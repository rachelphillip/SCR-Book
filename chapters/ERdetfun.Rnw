...
We are going to go with this simplistic encounter rate model for the moment, in order to develop some key ideas. We will return to the example to use a more realistic encounter rate model later. For simplicity, we also assume for the moment that all detectors are equally effective, so that the probability of an encounter at a detector depends only on the distance that the detector is from the activity centre, not on the particular detector being used.


We don't get to see usage pictures like that in Figure~\ref{fig:ER+detfun_habitat_encrate} during a survey, all we see are the detections at each detector, and from these we try to reconstruct something like the usage map, in addition to estimating animal density. In this chapter we focus on estimation the expected number of encounters as a function of distance from activity centre and leave density estimation aside until Chapter~\ref{sec:ch_spdetfn_integrating}. To keep things simple, we're initially going to work in one distance dimension rather than two. 
...
We can estimate $\lambda_0$ and $\sigma$ by fitting this function to the data in Figure~\ref{fig:ER+detfun_enc}. However, we need to take account of the survey duration to do this because the longer the survey, the higher the number of encounters will be. (Recall that \todo[inline]{in eqn. above, d had subscripts, now they have disappeared} $\lambda(d)$ is an encounter \textit{rate}, i.e. number of encounters \textit{per unit time}.) This is very easily done, because the expected number of encounters is proportional to the time spent surveying: if you survey for twice as long, you expect twice as many encounters. The expected \textit{number} of encounters on a survey of duration $T$ time units is just $\lambda(d)T$. This is shown in Figure~\ref{fig:ER+detfun_ENwithdists}.
...
Assuming that detections occur independently at each of $J$ detectors (as might be reasonable for camera trap surveys, or acoustic surveys), the capture history for individual $i$ is distributed as the product of independent Poisson random variables\todo{BCS: Nitpicky point, but to me ``distribution of a product of independent Poisson random variables'' sounds as though we are generating a bunch of Poisson random variables, then multiplying them together. That is, $X_i \sim \mathrm{Pois}(\lambda_i)$, and we want the distribution of $Y = \prod_i X_i$. However, this is actually trying to say that the \textbf{PMF} of the $i$th capture history is the product of Poisson \textbf{PMF}s.}:
\be
