...
\be
[\bm{\Omega}_n|\bm{S}_n]&=&\prod_{i=1}^n\prod_{j=1}^J\mbox{Bern}\left(n_{ij};p(d_{ij},T)\right)
...
We focus on the expected encounter rate function and detection function specification. To fit an SCR model you also need to specify (a) a model for activity centre density and distribution, and (b) because activity centres are unobserved, you need to specify a region within which the activity centres of the detected individuals could be, and outside of which they could not be. For (a) will use the default density model here, which assumes unknown but equal denisty of activity centres everywhere, and we will use a pre-prepared ``mask'' (an \texttt{secr} term) that specifies the region (b). Density models are dealt with in detail in Chapter~\ref{ch:SPmod} and masks are explained futher in Section~\ref{sec:??}.

The necessary data and fitted objects are obtained by loading an \texttt{.RData} object as follows:
<<ER-detfun-leopards-load, include = TRUE, echo = TRUE>>=
load("../analysis/objects/ERdetfun-objects.RData")
@ 
This loads the count capture history in \texttt{R} object \texttt{kruger.capt}, the corresponding binary capture bistory in \texttt{kruger.capt.bin} and the mask object in \texttt{kruger.mask} These are all objects created using the \texttt{secr} library.

\subsection{Example fit to count data}

To specify an encounter rate model we need to specify the functional form to use and to say how its parameters depend on covariates. We postpone consideration of covariates until later and so use the synax $\sim$\texttt{1}, which indicates no dependence on covariates.
...
er.hn.fit=secr.fit(kruger.capt,model=er.model,mask=kruger.mask,detectfn=detfn)
@


Lets also fit an encounter rate model with ``hazard rate'' shape:
\be
\lambda(d)&=&\lambda_0\left(1-\exp\left\{-\left[\frac{d}{\sigma}\right]^{-b}\right\}\right)
\ee
\noindent (called ``\texttt{HHR}'' in \texttt{secr}-speak), as follows
<<ER-detfun-leopardsexample-erhr-fit, eval = FALSE>>=
detfn="HHR"
er.hr.model=list(lambda0~1,sigma~1)
er.hr.fit=secr.fit(kruger.capt,model=er.model,mask=kruger.mask,detectfn=detfn)
@

We can select between these models on the basis of AIC$_c$ as follows
<<ER-detfun-leopardsexample-dfhn-aic, eval = FALSE>>=
AIC(er.hn.fit,df.hn.fit)
@ 
\noindent
and with a $\Delta$AIC$_c$ of only about 0.12, this suggests that there is very little to separate the two models. Point estimates and upper and lower 95\% confidence interval bounds for density, $\lambda_0$, $\sigma$ and the hazard rate parameter $b$ (called \texttt{z} in \texttt{secr}-speak) are show in Table ??\todo{how make this into a table?}.
...
The Gaussian model seems to be a very poor fit to the average encounter data, and in this plot looks a lot worse fit than the hazard rate model. However, we need to be cautious about over-interpreting this figure because it is based on the \textit{estimated} locations of activity centres, and these have substantial uncertainty associated with them, as shown in Figure~\ref{fig:ERdetfun-locest}. We can't really conclude a lot more from this plot than that the encounter rate does decline with distance, as expected, and that animals were very unlikely to venture more than about 15km from their activity centres over the surve period.
...
\subsection{Example fit to binary data}

<<ER-detfun-leopardsexample-dfhn-fit, eval = FALSE>>=
detfn="HN"
df.hn.model=list(D~1,g0~1,sigma~1)
df.hn.fit=secr.fit(kruger.capt,model=df.hn.model,mask=kruger.mask,detectfn=detfn)
@ 

<<ER-detfun-example1, echo = TRUE, fig.width = textwidth, fig.height = 0.25*textheight, out.width = add.in(textwidth), out.height = add.in(0.25*textheight), fig.pos = "tb", fig.cap = "The expected encounter rate $\\lambda(d)$ estimated from Kruger camera trap data by (a) specifying $\\lambda(d)$ to be Gaussian (solid curve), and (b) specifying $p(d,T=1)$ to be Gaussian (dashed curve). (Here the time unit, $T$, is the duration of the survey)">>=
par(mar =  c(4.1, 4.1, 0.1, 0.2))
plot(er.hr.fit,xval=seq(0,20000,length=200),lwd=3,xlab="Distance, d (m)",ylab=expression(lambda(d)))
plot(df.hr.fit,xval=seq(0,20000,length=200),add=TRUE,lwd=3,lty=2,col="gray")
@ 


\section{Traps in space and time}
