...
the number of times an individual is expected to encounter a detector
was assumed to depend on the detector's distance from its activity
centre (e.g., Eqn \eqref{eq:ER+detfun.lambda.hn}). Conditional on the
