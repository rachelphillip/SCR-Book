...
on the $k$th occasion is\todo{Does this need more explanation? Add
  plot with encounter rates and probabilities of detection at each
  trap.}
\be 
...
\begin{cases}
  ...
  p_{ijk}\frac{\lambda_{ijk}}{\sum_{j^{\prime} = 1}^J \lambda_{ij^{\prime}k}} & \text{if trapped by detector $j$.}
\end{cases}
\ee
