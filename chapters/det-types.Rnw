...
\texttt{secr}. Even if detectors are unable to count encounters (e.g.,
hair snares) or physically detain individuals (e.g., single- or
...
on the same survey (Table \ref{tab:det-types-table}).
...
model, any survey that uses traps that physically detain individuals
must comprise more than one occasion. As per Section
...
centre locations: 
\be 
    [\bm{\Omega}_n | \bm{S}_n] = \prod_{i = 1}^n \prod_{k = 1}^K [\bm{\omega}_{i \cdot k} | \bm{s}_i] \label{eq:det-types-multi-lik}
\ee

Here, $\bm{\omega}_{i \cdot k}$ contains the observed detection data
of the $i$th individual on the $k$th occasion. An individual may only
be detected by one of the detectors on a given occasion. Thus, for a
survey that uses $J$ multi-catch traps, there are $J + 1$ possible
outcomes that $\bm{\omega}_{i \cdot k}$ may take: we either observe
which of the $J$ traps detected invidiual $i$ on occasion $k$ (and so
$\bm{\omega}_{ijk} = 1$ if it was the $j$th detector made a
detection), or we observe that individual $i$ was not detected by any
trap (and so $\bm{\omega}_{i \cdot k} = \bm{0}$). The term
$[\bm{\omega}_{i \cdot k} | \bm{s}_i]$ is therefore the probability
associated with whichever of these outcomes that
...
the probability that th $j$th detector detected the $i$th individual
...
\ee
see Figure \ref{fig:det-types-multi}.
...
text(traps.x, traps.y, labels = format(round(ers, 3), nsmall = 3), c(-0.05, -0.15), col = "grey")
...
Therefore, putting Eqns \eqref{eq:det-types-multi-nondet} and
\eqref{eq:det-types-multi-det} together, we obtain
\be
[\bm{\omega}_{i \cdot k} | \bm{s}_i] = 
\begin{cases}
  1 - p_{i \cdot k} & \text{if not detected by any detector,} \\
  p_{i \cdot k}\frac{\lambda_{ijk}}{\sum_{j^{\prime} = 1}^J \lambda_{ij^{\prime}k}} & \text{if trapped by detector $j$.}
\end{cases}
\ee
...
probabilities of where one individual may be captured are affected by
...
prob.labels[c(3, 6)] <- ""
text(traps.x, traps.y, labels = er.labels, c(-0.05, -0.15), col = "grey")
text(traps.x, traps.y, labels = prob.labels, c(-0.05, 1.15))
@

Single-catch traps do not usually record when detections occur, and so
this makes it difficult to correctly specify $[\bm{\Omega}_n |
  \bm{S}_n]$. In order to appropriately specify the dependence between
individuals' detection records, one may consider the time each
detection took place as a latent variable. However, one must then
marginalise over these \todo{Back-reference to first mention of latent
  variables}, resulting in the evaluation of an integral of
...
parameter values that result in simulated data that match the closest
to the data obtained from the field. \todo{Is this an accurate enough
  explanation?}

Due to the computationally expensive procedure of simulating a large
number of data sets, estimation by inverse prediction is relatively
slow. Moreover, variance estimates (standard errors and confidence
intervals) are not available directly from asymptotic likelihood
theory, and one must again rely on simulation-based methods, which
further increases computation time. Other likelihood-based
tools---such as AIC for model selection and likelihood-ratio
hypothesis tests and confidence intervals---cannot be used.
...
resulting estimate for average density was remarkably robust in a
variety of situations, and only showed non-negligible bias at extreme
levels \todo{What value?}  of trap saturation. This estimator also
performed best when animal density is constant (or nearly so). These
findings were also echoed by \cite{Distiller+Borchers:15}.
...
parameters. Density estimates are typically negatively biased for
areas of high density, and positively biased for areas of low
...
using the inverse-prediction estimator from Section
\ref{sec:det-types-single-inverse}: the use of a likelihood function
allows (1) faster estimation, (2) variance estimates from asymptotic
likelihood theory, and (3) use of likelihood-based tools. Moreover, it
allows the use of \texttt{secr.fit}, rather than \texttt{ip.secr}, to
fit the model, which allows (1) estimation of a density
surface\footnote{Although recall that the estimated density surface is
  typically too flat; high-density areas are underestimated and
  low-density areas are overestimated. However, this may be preferable
  to a model that assumes a flat density.}, and (2) a choice from the
usual range of encounter rate and detection functions in the
\texttt{secr} package. These advantages are not inherent to the
...
%% Relatively robust, regain the use of likelihood machinery, can use
%% covariates in the software, etc.
...
\todo[inline]{Is this section all a bit too negative? It reflects
  concerns that I have about SCR that I have not really heard
  elsewhere.}
...
detectors, conditional on activity centre locations. There are a
variety of mechanisms that may violate these assumptions.

\subsection{Independence between individuals}
\label{sec:det-types-discussion-individuals}

We have already mentioned that detection data may not be independent
across individuals due to dependence in activity centre locations.
\todo{insert section references} Individuals may live in groups,
whereby individuals in the same group have the same (or similar)
activity centres and we would therefore expect them to have similar
detection records. Alternatively, individuals may be territorial and
have activity centres that repel each other. We would therefore expect
different individuals to be detected in different locations.
...
individuals. This results in a likelihood function that is likely to
be intractible.
...
location, is an important assumption for proximity detectors.

%% Conditional independence across traps (proximity):
%% Two traps near each other are likely to return similar counts

%% Poisson response assumption?
%% Why not try out a more general response distribution?