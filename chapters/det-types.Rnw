...
centre locations: 
\be 
    [\bm{\Omega}_n | \bm{S}_n] = \prod_{i = 1}^n \prod_{k = 1}^K [\bm{\omega}_{i \cdot k} | \bm{s}_i]
\ee
...
%% Inverse prediction and multi-catch trap analysis, ideally showing
%% that the estimates are very similar.
