...
\todo[inline]{SCR allows for 2 additional sources of recaptures relative to CR. Suggest:}
This is not true of SCR where you can get 1.recaptures at the same trap within occasions (if count rather than binary data are collected, and 2.\textit{recaptures over space} rather than, or in addition to, recaptures across occasions.
...
\todo[inline]{suggest using "study areas" rather than "locations", when referring to surveys separated in space, since "locations" often refers to e.g. activity centers and detectors.}
...
In such cases the individual surveys might be thought of as survey strata (although this is usually used to refer to surveys at different spatial locations, not at different times). In \texttt{secr}-speak they are called ``sessions'' (when surveys are separated by time and when they are separated by space).
\todo[inline]{I might have said that Program MARK and some CR literature use "groups", the DS literature often uses "strata", and Efford popularized the term "sessions", but in all cases, we analyze the data from all survey simultaneously, fit models with both common and different values of detection parameters across (some or all) sessions, and use model selection (or other?) criteria to determine the extent to which the data can be pooled across surveys to estimate common detection parameters. I think we could leave mention of sharing density model parameters until chapter 3 or later.}

We will generally follow \texttt{secr} terminology and call them ``sessions'' in this book.. (\cite{Royle+al:14} use both ``session'' and ``occasion'' to refer to what we call ``occasion'' in this book.)
\todo[inline]{We should be sure to do so. "Strata" is used below.}
...
\todo[inline]{Further to our discussion, the phrasing here seems to suggest both 1. that the surveys should be independent (so not possible to recapture animals across surveys?) to be modelled as sessions rather than as a single survey, and 2. that if there are no observed recaptures between surveys, they can be modelled as sessions.  To me, it doesn't make sense that it's OK to to be unable to recognize recaptures, but not OK to ignore them if we can recognize them. Matching up samples across surveys is likely to be time consuming and error prone, and there are advantages to modeling the different surveys as sessions, especially if they are separated in time. If I want to model my multiple surveys as sessions, must they be independent, or can I simply ignore the potential for recaptures across surveys (e.g. to avoid the need to use an open-population model, and despite some loss of information.)}
...
\todo[inline]{Here, mention (and demonstrate?) that in 'secr', covariates of detector locations and individuals are attributes of the capthist object, and may be read in with the data files or assigned afterwards, and that covariates of density are attached to the mask?}
