...
If it is possible to get recaptures of the same individual at more than one location \textit{and to recognise the recaptures as such}, then you could analyse the surveys as a single survey, with detectors in clusters (each survey corresponding to a cluster). If the time between the survey is large enough that the population size or distribution may have changed between surveys, this adds complexity and you may need to consider open-population SCR methods (Chaptet~\ref{ch:open}). 
...
In such cases the individual surveys might be thought of as survey strata (although this is usually used to refer to surveys at different spatial locations, not at different times). In \texttt{secr}-speak they are called ``sessions'' (when surveys are separated by time and whey they are separated by space). We will generally follow \texttt{secr} terminology and call them ``sessions'' in this book.. (\cite{Royle+al:14} use both ``session'' and ``occasion'' to refer to what we call ``occasion'' in this book.)
...
Failing to model the effects of such variables on encoutner rate, detection probability or density and distribution can result in biased inferences being drawn. (When such variables are used as explanatory variables in a statistical model we refer to them as ``covariates'' rather than as ``variables''.)  
