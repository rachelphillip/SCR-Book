...
In this case, each stratum is like a separate mini-survey that could be analysed on its own, without the other strata. But if some parameters may be shared across strata (if detection functions might share a common $\sigma$, for example), then a more parsimoniuous model (a model with fewer parameters) is obtained if we analyse data from all strata simultaneously. Because no recaptures occur across strata, the capture histories are independent between strata and the corresponding multi-stratum likelihood is just the product of the likelihoods for each stratum. Indexing stratum by $l=1,\ldots,L$, the stratified count and binary likelihood functions are:
\begin{svgraybox}
...
Encounter rates and detection probabilities often depend on variables associated with the detectors, the environment, individual animals and other things. While there is no single correct way to model the dependence of encounter rate and detection probability on the covariates, it is commonly done
\ben
...
\subsection{Link function formulation for encounter rate and detection probability}

The GLM \{link function; linear predictor\} form for modelling detection probability is commonplace in non-spatial capture-recapture literature. An alternative \{intercept; scale\} form, in which $g_0$ or $\lambda_0$ is the intercept and a ``key function'' or ``kernel'' with a scale parameter $\sigma$, is used in the distance sampling literature, and has been adopted by SCR models. In Eqn~\eqref{eq:ER+detfun.p.hn}, for example, the intercept is $g_0$ and the key function is $\exp\{-d^2/(2\sigma^2)\}$, while in Eqn~\eqref{eq:ERdetfun-hr}, the intercept is $\lambda_0$ and the key function is $[1-\exp\{-(d/\sigma)^b\}]$, where $d$ is distance from a detector to an activity centre. 

Because the capture history likelihoods obtained above are just products of Poisson (in the case of count data) or Bernoulli (in the case of binary data) likelihoods, one might expect that the Poisson mean ($\lambda(d)$) and Bernoulli mean $(p(d)$) would be linked to the explanatory variable $d$ using the GLM inverse link functions for these two distributions (the inverse log link, and the inverse logit or cloglog link functions, respectively -- see Table~\ref{tab:ERdetfun-invlinks}). 

Some encounter rate and detection function forms that are commonly used in SCR models can be written in GLM \{link function; linear predictor\} form, but not all can be. 

The Gaussian model for expected encounter rate (Eqn~\ref{eq:ER+detfun.lambda.hn}) is and example in which the mean encounter rate is readily formulated like this. It can be written using an inverse log link function as
\be
...
When the intercept of an encounter rate model that can be written in terms of an inverse log link, is made to depend on covariates via and inverse log link, this just adds a term to the linear predictor. For example, if we make $\lambda_0$ depend on a covariate $z$ thus: $\lambda_0(z)=\exp\{\beta_0+\beta_z z\}$, then Eqn~\eqref{eq:ERdetfun-negexp-lambda} becomes $\lambda(d,z)=\exp\{\beta_0+\beta_d d^2 + \beta_z z\}$. 
