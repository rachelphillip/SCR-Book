...
\todo[inline]{Mention earlier in this section that sessions should only be considered independent if there are no recaptures between them. However, I'm confused about the case where they cannot be recognized as such. Where individuals are identified from DNA or images, recaptures at different times might not be recognizable (because slight differences in e.g. equipment calibration cause gynotypes from the same individual to differ between surveys, and natural marks may evolve). In these cases, we treat the surveys as indepdent, even if some of the same animals are detected, so why not do the same even if we can recognize some of these recaptures?  I also note that the longer the time between surveys, the fewer of the same individuals will be present. All of this applies only to surveys separated in time, not space}
...
If it is possible to get recaptures of the same individual during more than one survey \textit{and to recognise the recaptures as such}, then you could analyse the surveys as a single survey, with detectors in clusters (each survey corresponding to a cluster). If the time between temporally replicated surveys is long enough that the population size or distribution may have changed between surveys, this adds complexity and you may need to consider open-population SCR methods (Chapter~\ref{ch:open}). 
\todo[inline]{EH: Two options if population size or distrib may have changed: (1) treat the surveys as independent, even if some of the same animals are present and detected. }
...
\todo[inline]{Not sure if it's useful to introduce the term strata, or to say it's usually used to distinguish locations. I think Program MARK and some CR literature uses "groups", the DS literature uses "strata", and Murray popularized the term "sessions", but in all cases the data can be separated in space, time, or even within populations (people use "groups" and "sessions" to model sex differences), and the appropriate degree of partitioning or aggregating of the data (including whether D is constant or differs between surveys) is another another model selection question? Could we equate sessions to both "strata" and "groups" here, and strike the parenthetic statement about different locations?}
