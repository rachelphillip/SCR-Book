...
<<area-transect-integral, echo = FALSE, fig.width = textwidth, fig.height = textwidth, out.width = add.in(textwidth), out.height = add.in(textwidth), fig.pos = "tb", fig.cap = "Placeholder caption.">>=
res <- 700
...
z.range <- c(0, max(z))
par(xaxs = "i", las = 1, mar = c(2, 3, 0, 0) + 0.1, mfrow = c(2, 2))
area.x.lims <- c(0.1, 0.3)
area.y.lims <- c(-0.3, -0.1)
area.vertices <- matrix(c(rep(area.x.lims, each = 2), area.y.lims[2], rep(area.y.lims[1], 2), area.y.lims[2]), ncol = 2)
plot.new()
plot.window(xlim = c(-0.5, 0.5), ylim = c(-0.5, 0.5), asp = 1)
image(x = unique.x, y = unique.y, z = z, zlim = z.range, col = rgb(0.15, 0.15, 0.15, (0:1000)/1000), add = TRUE)
points(0, 0, pch = 4, cex = 2, lwd = 2)
polygon(area.vertices, border = "red", lwd = 2)
box()
plot.new()
box()
plot.new()
...
par(new = TRUE)
z.out <- z
z.out[unique.x >= area.x.lims[1] & unique.x <= area.x.lims[2],
      unique.y >= area.y.lims[1] & unique.y <= area.y.lims[2]] <- NA
perspmat <- persp(x = unique.x, y = unique.y, z = z.out, zlim = c(0, max(z)),
                  shade = 0.75, col = "lightgrey", theta = 30, phi = 30,
                  border = NA, axes = FALSE)
par(new = TRUE)
z.in <- z
z.in[!is.na(z.out)] <- NA
perspmat <- persp(x = unique.x, y = unique.y, z = z.in, zlim = c(0, max(z)),
                  shade = 0.75, col = "red", theta = 30, phi = 30,
                  ltheta = 0, lphi = 0, border = NA, axes = FALSE)
plot.new()
box()
@ 
