...
density of cues per unit distance of the transect travelled that are
found at distance $d$ from an activity centre for line transects.
...
For area searches, we redefine $\Lambda(d)$ as the probability density
...
distance $\Lambda(d)$ from the activity centre, there is a
$\Lambda(d)$ probability that the animal is within that area (i.e., it
...
area. Likewise, for line transect surveys, we redefine $\Lambda(d)$ so
...
\begin{equation}
  \Pr(\omega_{ijk} = 1 | \bm{s}_i) = p_d \int_{A_j} \Lambda(d(\bm{x},
  \bm{s})) d\bm{x}, \nonumber
\end{equation}
and so we obtain
\begin{equation}
...
\begin{equation}
  [ \bm{y}_{i \cdot k} | \bm{\omega}_{i \cdot k}, \bm{s}_i ] = \frac{\Lambda(d(\bm{y}_{i \cdot k},
    \bm{s}_i))}{\int_{A_j} \Lambda(d(\bm{x}, \bm{s}_i))
