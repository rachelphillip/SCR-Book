...
<<area-transect-area-setup, eval = FALSE>>=
...
traps <- make.poly(polylist = list(area1, area2), 
                   exclusive = FALSE)
@ 
...
\todo[inline]{Need to put something in here to explain how to read in
  ...
  yere. Also need an example data set to fit, below.}
...
`trap'. Each detection is allocated to the nearest pixel. These traps
either act like proximity detectors (for cue-based surveys) or
multi-catch traps (for individual-based surveys).
...
area.capt.discrete <- discretize(traps, spacing = 10, 
...
that are negligible in size.

For the \texttt{outputdetector} argument, recall that the
\texttt{secr} package calls proximity detectors that record counts
`count' detectors, while `proximity' detectors are only capable of
recording binary response data.
